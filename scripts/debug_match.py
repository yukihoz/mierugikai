...
def normalize(text):
    ...
    # wait, replace \r\n with \n
    # let's just strip and replace all whitespace for matching key
    import re
    if text:
...
def main():
    ...
    with open(TEMPLATE_CSV, 'r', encoding='utf-8') as f:
        ...
        for i, row in enumerate(reader):
            ...
            # Use only first 50 chars of body for robust matching
            body = body[:50]
    ...
    with open(MASTER_CSV, 'r', encoding='utf-8') as f:
        ...
        for i, row in enumerate(reader):
            ...
            meeting = normalize(row.get('会議の名称', ''))
            if '令和7年決算特別委員会' in meeting or '令和7年　決算特別委員会' in row.get('会議の名称', ''):
                target_indices.append(i)
                target_rows.append(row)
                
    print(f"Found {len(target_rows)} target rows.")
    ...
    if unmatched:
        ...
        for r in unmatched[:3]:
            # print original to see what we missed
            print("ORIG MEETING:", repr(r.get('会議の名称')))
            print("ORIG SPEAKER:", repr(r.get('発言者')))
            print("ORIG BODY[:50]:", repr(r.get('発言内容', '')[:50]))
            
    print("Example template key:")
    print(list(template_order.keys())[0])
